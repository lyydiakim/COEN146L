...
* Lydia Kim
* Date:         5/15/2023
* Title:        Lab5 - Server Side 
* Description:  This program recieves a file from a client through UDP and checks for correct checksum, sequence, and ack number.
*/
...
Packet serverReceive(int sockfd, struct sockaddr *address, socklen_t *addrlen, int seqnum) {
    ...
    while (1) {
        ...
        if (packet.header.cksum!=e_cksum) {
            ...
            serverSend(sockfd, address, *addrlen, !seqnum);  //resend packet
        ...
        //if the ckechsum and sequence numbers are correct send ack with the right sequence number
            ...
            serverSend(sockfd, address, *addrlen, seqnum); //send ack
            break;
        }
    }
    return packet;
...
int main(int argc, char *argv[]) {
	...
	clienAddr.sin_port = htons(port);
	clienAddr.sin_addr.s_addr = htonl(INADDR_LOOPBACK); 
