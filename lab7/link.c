...
void * receive_info(void *arg)
{
	...
	while (1)
	{
		...
		int source_id = ntohl(received_packet[0]);
		int destination_id = ntohl(received_packet[1]);
		int new_cost = ntohl(received_packet[2]);
		
		// Update costs matrix
		pthread_mutex_lock(&lock);
		costs[source_id][destination_id] = new_cost;
		costs[destination_id][source_id] = new_cost;
		pthread_mutex_unlock(&lock);
